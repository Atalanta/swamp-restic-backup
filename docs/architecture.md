...
C4Component
    Container_Boundary(model, "Model definition") {
        ...
        Component(spawn, "_lib/spawn.ts", "module", "Sole owner of Deno.Command and Deno.env. Exports SpawnEffect (injectable seam for test injection), invokeResticInternal (secret-injecting, used only by commands.ts), invokeResticNoSecrets (no-secrets probe). realSpawn is module-private.")
        Component(commands, "_lib/commands.ts", "module", "Typed per-command invoker entries (invokeResticCheck, invokeResticPrune, invokeResticSnapshots, invokeResticForget, invokeResticBackup, invokeResticInit, invokeResticCatConfig, invokeResticRestore) and probeResticCapability. Each entry accepts an optional spawn:SpawnEffect for test injection. No generic argv:string[] export — method modules pass typed inputs. invokeResticRestore requires SafeRestoreTarget.")
