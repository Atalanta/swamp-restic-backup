    ...
    model {
        ...
        ext = softwareSystem "@atalanta/restic-backup" "Swamp model extension: typed methods over restic to back up .swamp/ runtime evidence to B2." {
            model = container "Model definition" "The @atalanta/restic-backup/repository model: a thin registration shell (restic_backup.ts) plus modules under _lib/, with boundaries enforced by the import graph." {
                ...
                invoker = component "_lib/invoker.ts" "Typed per-command invokers (invokeResticCheck, invokeResticPrune, invokeResticSnapshots, invokeResticForget, invokeResticBackup, invokeResticInit, invokeResticCatConfig) plus invokeResticRestore (SafeRestoreTarget) and invokeResticNoSecrets/probeResticCapability; parse helpers; ResticResult. Sole owner of Deno.Command (spawnRestic is module-private). No generic argv:string[] export — method modules pass typed inputs."
        ...
        methodmodules -> invoker "passes typed inputs to per-command entries"
        ...
        invoker -> restic "spawns typed-command argv with --json, secrets in env"
