    ...
    model {
        ...
        ext = softwareSystem "@atalanta/restic-backup" "Swamp model extension: typed methods over restic to back up .swamp/ runtime evidence to B2." {
            model = container "Model definition" "The @atalanta/restic-backup/repository model: a thin registration shell (restic_backup.ts) plus modules under _lib/, with boundaries enforced by the import graph." {
                ...
                spawn = component "_lib/spawn.ts" "Sole owner of Deno.Command. Exports SpawnEffect (injectable seam), realSpawn (real Deno.Command-backed SpawnEffect — fully-built env, injects no secrets), and invokeResticNoSecrets (no-secrets PATH-only probe). The secret-injecting invokeResticInternal is module-private in commands.ts — spawn.ts exposes no secret-bearing raw-argv entry."
        ...
        commands -> spawn "builds secret env, delegates to realSpawn / invokeResticNoSecrets"
